...
const Q1_OPTIONS: { value: Years; label: string }[] = [
  { value: 'new',     label: 'Still finding my feet' },
...
export function QuizTestWelcome({ onClose, onStartQuiz }: QuizTestWelcomeProps) {
  ...
  if (step === 'welcome') {
    return (
      <HeroScreen
        ...
        title="Hello"
        body={[
          "Here's twelve questions to see where you're at.",
          'Plus two quick ones to get us started.',
        ]}
        buttonLabel="Dig in"
        onButton={() => setStep('q1')}
      />
  ...
  return (
    <HeroScreen
      onClose={handleClose}
      title={CIRCLE_DISPLAY[circle]}
      body={[CIRCLE_DESCRIPTION[circle]]}
      circleActiveRing={CIRCLE_TO_RING[circle]}
      buttonLabel="Take the quiz"
      onButton={() => onStartQuiz?.(circle)}
    />
  );
}
